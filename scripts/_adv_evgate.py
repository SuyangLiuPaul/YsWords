...
# EXPERIMENTAL (measurement only): admit a two-character ABBREVIATION
# mid-sentence when unambiguous citation evidence follows it directly —
# a 第, a chapter mark, or an explicit verse. Same evidence the
# bare-numeral guard already demands after the fact; this asks for it
# before admission instead.
def build_cjk_infix_abbrev_pattern() -> str:
    def is_cjk(a: str) -> bool:
        return any("一" <= c <= "鿿" for c in a)

    admitted = {a for a in ALIAS if is_cjk(a)
                and len(a) == 2 and a[1] in _ABBREV_TAIL}
    ordered = sorted(admitted, key=lambda s: (-len(s), s))
    return r"(?:" + "|".join(re.escape(a) for a in ordered) + r")"


CJK_INFIX_ABBREV_RE = build_cjk_infix_abbrev_pattern()

_ABBREV_EVIDENCE_RE = (
    r"\.?\s*(?:第"
    rf"|(?:\d+|{_CN_NUM_RE})\s*[章篇]"
    rf"|(?:\d+|{_CN_NUM_RE})\s*[:：]"
    r")"
)
...
# not — see the bare-numeral refusal there. A two-character ABBREVIATION
# joins the same branch, but only when `_ABBREV_EVIDENCE_RE` confirms a
# citation follows it — see the comment above.
BOOK_START_RE = (
    rf"(?:\b|(?<=[一-鿿])"
    rf"(?={CJK_INFIX_BOOK_RE}|{CJK_INFIX_ABBREV_RE}(?={_ABBREV_EVIDENCE_RE}))"
    rf"(?P<infix>))"
)
