...
> Last updated: 2026-06-14 — **v1.3.72 (ALL 6 sites incl. prod + native devices)** — iOS themed-icon ACTUAL root cause fixed: the `yswords/ios_icon` channel was registered on the wrong binary messenger (UIScene migration) so every call silently `MissingPluginException`'d — now uses `engineBridge.applicationRegistrar.messenger()`. (v1.3.70's icon work was correct but moot until this.) Prior: AI verse panel confirm-first (v1.3.71). Prior: AI verse panel "ask a question" box + scripture layout (v1.3.68); dark mode tracks theme colour, accent softened to a light pastel so the "读经" hero card isn't harsh (v1.3.69). See v1.3.x highlights. — **v1.2.95 → v1.3.66** — v1.3.66 on **all 6 sites incl. prod** (full-audit pass 2: responsive tests for ALL 19 pages × 4 widths, zero overflow; SearchPage post-dispose poll fixed; 303 tests; CI 6/6 green). ⚠️ **launchd shim post-mortem:** the 2026-06-10 exec-shim BROKE the 04:00 job on its first run (2026-06-12 04:00, exit 126 — TCC blocks launchd bash from opening scripts in ~/Documents: `Operation not permitted`). Reverted to a FULL COPY at `~/.config/yswords/scripts/` (byte-identical to repo, includes the macOS xcodebuild fix); `release_web.sh` now auto-syncs the copy on every release so it can't drift again. Native devices missed the 06-12 04:00 update (iPhone/iPad v1.3.62, Mi Pad v1.3.60, macOS v1.3.62) — tonight's run brings all to latest. macOS on v1.3.62 (rebuild needs `flutter build macos --config-only` + `xcodebuild -allowProvisioningUpdates`; reinstall script patched). iPad/iPhone/Mi Pad on v1.3.60 — device-side blockers (iPad 3002 / iPhone asleep / Mi Pad not advertising ADB); overnight launchd job (now macOS-hardened) brings them to latest. v1.3.61→64 are web-only behaviorally — native unchanged except the version string. **PROD note:** `yswords` (EN) ships the international build; `yswords-cn` ships the **CHINA_MODE** build (no Firebase/Google sign-in) — `release_web.sh` now builds BOTH bundles. (See v1.3.x highlights below for v1.3.46–v1.3.59; the long summary that follows is the v1.2.95→v1.3.45 history.) — **v1.2.95 → v1.3.45 — PageView root-cause refactor, themed iOS/Android/macOS/web app icons, per-category notification scheduler, daily-verse rotation fix, comprehensive perf sweep, navigation dedup, release tooling, YHWH name restoration across all aux assets, version-gap UI, Chinese exegesis prompt fix, split-pane chrome cleanup, paragraph-group Selector + scroll-state ValueNotifier refactor, haptic feedback + macOS Cmd shortcuts, 朗读/TTS feature fully removed, avatar decode caps + responsive reading-column max-width, cross-platform error reporting via Resend, robustness bundle (CI + silent-error reporting + iOS memory pressure), test coverage on new infra, security bundle, PWA install affordance + analyze strictness restored, highlights/notes export, CSP removed after whack-a-mole loop, BYOK→shared-key auto-fallback, bundled Noto Sans CJK subset for tofu-glyph fix on web, mini-header backdrop + scroll-to-top fix, wider reading-column caps for large tablets, Strong's stub-gloss recovery + dedupe, tap-top-to-scroll fixed for Dynamic Island, chrome buttons unblocked, note glyph one-tap-to-edit, CN-build native coexist scaffolding (Android flavors + iOS bundle-ID patch script), note timestamp honesty (no fake "just now"), version-switch race fix + locale-aware default version (en→NASB, zh-Hant→CUVS-YHWH-TR, zh-Hans→CUVS-YHWH), cross-device sync for last-read position + all user prefs (lastRead + userPrefs JSON blobs, newest-timestamp-wins via paired epoch-ms keys), seed-write fix for v1.3.41 sync, sermon-resume cross-device sync (sermonId field in lastRead blob with full read/write/refresh wiring), and EMERGENCY HOTFIX v1.3.45 — RTDB sync was 100% broken on prod for any user with legacy plan.* prefs (reading-plan feature removed in v1.2.69 but plan.activeId/startMs/useDate/completed.* keys still in sync schema; RTDB rejects dots in keys; all syncs silently failed for affected users including the cause of every "Why all not synced yet" complaint). Stripped plan.* + added defensive `_isInvalidRtdbKey` guard at snapshot collection AND right before `FirebaseDatabase.set()`.**
>
