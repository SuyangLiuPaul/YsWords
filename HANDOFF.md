...
> • **⚠️ fuyindiantai.org is NOT a fourth source and must not be added as one.** It is fydt.org under its former domain (it 301'd to fydt.org through 2025) and **its DNS is currently broken** — Google and Cloudflare both return SERVFAIL because its NS records still point at `ns1/ns2.fydt.org`, which stopped serving the zone when fydt.org moved to DigiCert DNS. Its songs are already in the catalogue under `fydt`. Worth telling the church: anyone holding an old fuyindiantai.org link (print material, bookmarks) currently gets nothing.
