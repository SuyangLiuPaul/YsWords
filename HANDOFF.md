...
> • **🔴 `web/song_media_sw.js` NEVER RAN. Not once, since the day it shipped.** It was registered with `scope: '/song-media/'` — and a worker's scope decides which **pages** it controls, not which requests it is allowed to see. The app is served from `/`, so the worker controlled no clients and its `fetch` handler was never invoked. Verified in a browser on the deployed dev site: `navigator.serviceWorker.controller` was `null` and every media request came back with `server: Netlify`. Every claim made about it — "media you have played is cached and served offline" — was false.
> • **Widening it to `/` does not work either, so the worker is DELETED.** Flutter's own generated service worker claims root scope, and two scripts cannot share one scope: the later `register()` simply replaces the earlier. Competing for it would trade offline media for a stale app shell (the console showed exactly that fight — `prepareServiceWorker took more than 4000ms`). index.html's sweep now unregisters **every** worker including the old media one, which is what removes it from installs that already carry it; the `song-media-` **cache** exemption stays, because that bucket is now where downloads live.
> • **Web offline downloads now exist for real** (`lib/services/song_download_web.dart`, previously a no-op stub with `isSupported => false`, which meant the download UI was hidden on web — against what the user asked for). Audio is fetched through the same-origin proxy, stored in Cache Storage, and **played back through a blob URL**: no worker in the path, nothing to claim, works with the tab offline, and Range requests never arise because a blob URL is not fetched over HTTP. Verified end-to-end in a browser on dev — cached 8.5 MB, read it back, created the object URL, played it, position advanced, correct duration, no error.
> • Blob URLs are built **at init, for every downloaded song**, not on demand. The player resolves a source SYNCHRONOUSLY, and reading Cache Storage at play time would put an `await` between the user's tap and `play()` — precisely what costs the gesture activation iOS requires. An object URL is only a handle to a blob the browser already holds on disk, so a few hundred cost nothing.
> • Downloads are reconciled against the cache on every launch, exactly as the native service reconciles against the filesystem: the browser may evict without telling us, and an index still claiming a song is downloaded would send someone on a plane to a dead play button. `navigator.storage.persist()` is requested on first download — best-effort, not a guarantee. Quota on the test device: **4.2 GB**.
> • **🔴 `window.open(url, '_blank')` strands users in an installed PWA.** There is no tab for `_blank` to open into, so iOS loads the URL in the app's OWN standalone window — which has no address bar and no back button. Replaced with a synthetic anchor click (`target=_blank`, `rel=noopener`), which iOS hands to Safari as a separate app and leaves the PWA alone.
> • **CDC's Classic Piano Hymns — 15 songs — were invisible to the sync** (yswords-data `c44587ac4`). A user said songs were still missing and was right. Three things hid them: the pages are `h01`…`h15`, not the four-digit `[a-z]\d{4}` every other CDC song uses; the audio is under `/files/hymns/mp3/`, not `/files/music/mp3/`; and none of them appear in `integrated-list-songs`, the only index the sync read. All 15 have an mp3 **and** a PDF lyric sheet.
> • **Checked at the same time and found complete, so nobody repeats the search:** cgdc.hk is exhausted — its four camp songbooks hold 14/17/18/14 mp3s, the catalogue has all 63, and the site's entire media library contains exactly 63 audio files across 456 items; it has 4 albums, 0 posts, and 10 non-song branch pages. The CDC D/E catalogue covers every code on the site including the deep pager pages (283 in the catalogue vs 256 on the index — a superset). Every cdc, cgdc and fydt row already carries audio.
>
> Last updated: 2026-08-10 — **v1.4.24 → v1.4.25 — the lock-screen skip buttons were three separate bugs. dev/qat only; PROD HELD (still on v1.4.5).** analyze clean · **535/535 tests** · **4/4 on an iOS simulator**.
> • **🔴 `MediaAction.seekForward` / `seekBackward` must NOT be advertised unless the intervals are configured.** audio_service maps them to MPRemoteCommandCenter's skip-by-interval commands and attaches a handler **only when `fastForwardInterval` / `rewindInterval` are non-zero** (`AudioServicePlugin.m`, `case AFastForward`). This app never set them, so iOS was told the app supports skip-by-interval and given nothing to call — the ⏪ ⏩ a user photographed on their lock screen, drawn and dead. Removed; `MediaAction.seek` (the scrubber) works and stays.
> • **🔴 On WEB, a control that is never published is a handler that never exists.** `audio_service_web.setState` registers its Media Session action handlers **by walking `state.controls`** — `previoustrack` only appears in that loop if `MediaControl.skipToPrevious` is in the list. Ours were gated on `hasPrevious` / `hasNext`, so the first track of a queue never registered them. Skip controls are now published whenever the queue holds more than one item; `hasNext`/`hasPrevious` still govern what the handler *does*.
> • **🔴 …and the queue was usually one song long.** The row play button called `toggle()`, which builds a single-item queue — nothing to skip to, by construction. Tapping play now starts the **visible filtered list** positioned at that song. Tapping the row that is already playing still just plays/pauses so it does not restart mid-song.
> • **`SongQueue.fromSongs` gained `startSongId`; prefer it over `startIndex`.** `startIndex` counts into the RESULT, which has had every song with no playable track dropped — so a row number taken from the visible list lands on the wrong song as soon as anything above it was unplayable, and an instrumental-only playlist with `skip` can drop most of the list. Both call sites (Songs page, playlist detail) now pass an id.
> • **`SongPlaybackEngine.dispose()` could hang forever on native.** `AudioPlayer.dispose()` awaits the same start-up future that failed for want of a platform side, and it **never completes** rather than failing — it wedged an entire test file before anyone noticed, and would do the same to a shutdown path on a device with no audio plugin. Skipped when start-up is known to have failed, capped at 3s otherwise.
> • **Apple Watch pause, and "tapping the Now Playing widget opens the wrong site" — both are the PWA, not the app.** When audio comes from a web page, the OS media controls call back into JavaScript that iOS may have suspended, and the widget hands control to **the browser**, which surfaces whatever tab/PWA it last had in front (the user landed on an unrelated PeopleStories page). Neither is fixable from our side on web. **The native iOS build does not have either problem** — its commands are handled in native code by audio_service, and the widget returns to YsWords. Point users who want lock-screen / Watch / CarPlay control at the native app (`tools/yswords-ios-reinstall.sh`), not the PWA.
> • Guarded by `test/song_media_controls_test.dart` — asserts skip controls exist on the FIRST track of a multi-song queue, that skip-by-interval is never advertised, and that a genuine one-song queue still offers no skip.
>
...
> Last updated: 2026-08-10 — **v1.4.23 → v1.4.24 — Songs: app-wide player, a web engine without Web Audio, and the four gaps that were left. dev/qat only; PROD HELD (still on v1.4.5).** analyze clean · **532/532 tests**.
> • **🟠 iOS web audio: the fix from the v1.4.23 note is now IMPLEMENTED — and still UNVERIFIED on a device.** That note ended "if iOS is still silent, the fix is a web-only playback engine using a plain `HTMLAudioElement` with no AudioContext in the path". That engine now exists: `lib/services/playback/song_playback_engine.dart` is a conditional export (`_web.dart` / `_native.dart`), and the web side is a bare `HTMLAudioElement` — no AudioContext, no `crossOrigin` (the same-origin `/song-media/*` proxy makes it unnecessary). One element is created and reused for the whole session, because iOS grants play permission to *that element* on its first user-gesture `play()` and the grant sticks. **`audio_service` is back on web**: v1.4.22 disabled it on a theory that turned out to be wrong (the silence persisted without it), and audio_service_web only manages Media Session metadata — it never touches playback. **Someone still has to open the PWA on an iPhone and confirm sound comes out.** Native audio was never affected.
> • **Playback survives navigation.** The mini-player lived on the Songs page, so leaving it took the only transport with it — the user reported this as "跳到其他界面也是听不了了", and from outside a vanished player and stopped audio look identical. `lib/widgets/global_mini_player.dart` is wrapped around the whole app in `main.dart`'s `builder:`, not added per page, so a page added later cannot forget it.
> • **Album / year filter for cgdc.** cgdc releases one album a year (`2026 SAIL 乘風破浪`, `2025 Nearer 更亲近`, …) and that congregation refers to songs that way. `SongService.distinctAlbums` sorts on the leading year descending, so next year's album lands at the front with no code change; the section hides itself for the three sources that have no albums. `PlaylistFilter` carries `album` too (missing key defaults to `'all'`, so playlists saved earlier match exactly what they used to).
> • **The Apply button in the filter sheet was still scrolling away.** v1.4.20 moved it to the top but left it *inside* the `SingleChildScrollView`, so it was above the sections and scrolled with them; adding the Album section made the sheet tall enough that scrolling to a year chip put Apply ~260px off the top of the screen. The header now sits outside the scroll view. **A widget test found this by tapping thin air** — `songs_filter_sheet_test.dart` now scrolls the body and asserts Apply is still on screen.
> • **Playlists have contents.** `reorder` and `removeSong` had unit tests and no UI at all: a song added by mistake was stuck there forever. New `lib/pages/song_playlist_detail_page.dart` — drag to reorder, remove with undo that restores the *original position*, and read-only for smart playlists (their order comes from the catalogue, so a drag handle would promise something the model cannot keep). It takes an **id, not a snapshot**, for the same reason the service re-reads with `_live` before every mutation. Tapping a playlist row now opens it; play moved to its own button, because a mis-tap that starts 47 songs in the car is worse than one that shows a list.
> • **The queue is visible.** "3 / 47" was dead text — the only evidence a queue existed. It is now a control that opens the playing order, scrolled to the current track, with tap-to-jump. Reaching track 30 no longer means pressing next 27 times.
> • **Single-song download.** Downloading was bulk-only; you could take a whole filter offline but not the one hymn on screen, which is the commonest case before a flight. One button in the detail sheet covering all four states (download / progress-and-cancel / done-tap-to-remove-with-undo / retry). Native only.
> • **The native engine no longer throws when the audio plugin is missing.** `audioplayers` reports a failed platform hand-shake by completing an internal future with the error; nothing was awaiting it at that moment, so it became an unhandled async error that took down the enclosing zone. Now a harmless `setVolume` probe starts awaiting in the same turn, the failure is latched and reported through `onError`, and every later command no-ops instead of throwing — a dead audio stack no longer abandons the whole playlist on track one.
> • **Songs now refresh DAILY, not weekly** (yswords-data `dcf1a8e45`, 18:00 UTC; this repo's snapshot pull follows at 02:00). A hymn posted on a Tuesday used to wait until the following Monday. **`--verify` stays weekly** — it is ~1400 HEAD requests and the run that took 2h12m was entirely verification; link rot is slow enough that finding it on Sunday costs nothing. A manual run always verifies. The app's own 12h `minRefreshInterval` is unchanged.
> • **CI: `flutter pub get` is retried 3×.** A run failed with `Authentication error (403)` from `HostedSource._fetchAdvisories` — pub.dev's advisory endpoint, queried *after* the dependency solve had already succeeded. Nothing to do with our pubspec, and it cleared on its own.
> • **🔴 DEPLOY TRAP — `flutter build web --output=build-cn` EMPTIES `build/web`.** Building intl first and China-mode second left `build/web` holding only `index.html`, `assets/`, `canvaskit/` and `flutter.js` — **no `main.dart.js`, no `flutter_bootstrap.js`, no `manifest.json`** — while `build-cn` was complete. The intl build had printed `✓ Built build/web`, and `netlify deploy` uploaded the shell happily, so **yswords-dev and yswords-qat served a blank page** until it was spotted (`index.html` requests `flutter_bootstrap.js`, the SPA catch-all answers with `index.html`, and Chrome refuses it for MIME type `text/html`). Rebuilding intl AFTER the cn build produced a complete directory and both sites recovered. **Always build China-mode first, intl second — and before deploying, check the output has `main.dart.js`:** `ls build/web/main.dart.js build/web/flutter_bootstrap.js`. A one-line post-deploy check catches it too: `curl -s https://<site>.netlify.app/flutter_bootstrap.js | head -c 40` must be JavaScript, not `<!DOCTYPE html>`.
> • **✅ NATIVE IS VERIFIED ON A DEVICE NOW — new `integration_test/songs_native_test.dart`.** Everything under `test/` runs on flutter_tester, where audioplayers, path_provider and shared_preferences have no platform side at all, so downloads, playback and playlist persistence had 500-odd green tests and had **never written a file, opened an audio stream or survived a real preference round-trip**. Run on an iPhone 17 Pro simulator, all four pass: a download writes a real mp3 (byte count matches the index, the `.part` name is gone, and delete removes the file rather than just the index entry); the native engine opens a stream, reports a duration and **advances the position**, and pause stops the clock; a playlist survives reorder + remove across a cache-clearing reload; shuffle keeps the playing track first. Command: `flutter test integration_test/songs_native_test.dart -d <udid>`. **Deliberately NOT in CI** — it streams from the church servers, which is what the sync workflow works hard to avoid doing from GitHub runners.
> • Two traps that test surfaced, both now handled in it: **(1)** it must pick a host that answers. fydt.org and christiandiscipleschurch.org still time out from this machine (cgdc.hk answers), so `catalogue.first` tested the network, not the app — the first run failed with `AVPlayerItem.Status.failed` for no better reason than an unanswered request. It now probes one URL per host and skips with a plain message if none reply. **(2)** audioplayers publishes position from a **FramePositionUpdater — a frame callback** — so a plain `Future.delayed` lets audio decode while the position stream stays silent; the wait has to be `runAsync` (wall clock) **plus** `tester.pump()` (frames), or a perfectly good stream reports "position never advanced".
> • **🔴 A refused `play()` used to kill the entire queue.** On web, `play()` rejects with `NotAllowedError` when the call did not come from a user gesture — *precisely* the iOS failure being chased. The handler could not tell that from a 404, so it marked the song dead and advanced… into the same refusal, again, for every remaining track, ending with silence and an empty queue. New `PlaybackBlockedException` (`lib/services/playback/playback_blocked.dart`, re-exported from the engine façade) is raised when `play()` rejects **and `element.error` is null** — a decode/network failure always sets it, a policy refusal never does, and that beats reading `name` off a JS value whose Dart representation differs per compiler. The handler now stops, keeps the song, and shows `songsPlaybackBlocked` ("tap play again") instead of blacklisting the catalogue.
> • Also fixed while reading it: the web engine compared `_el.src` (which the element resolves to an **absolute** URL) against the **relative** `/song-media/…` it is handed, so the two never matched and every play re-assigned `src` and re-downloaded a file the browser already had.
> • **The `/song-media/*` proxy is verified live on dev** — fydt, cdc and cgdc all answer `206` with `audio/mpeg` and a correct `Content-Range`, including cgdc's percent-encoded Chinese filenames, and a bare `HTMLAudioElement` pointed at one plays with the position advancing. So the proxy, the range requests and the engine's approach are all confirmed; what remains unconfirmed is only iOS's gesture behaviour.
> • **Still open:** iOS PWA sound needs a real device; native verification of downloads / playlists / shuffle / sleep timer has never been done on hardware; prod remains on v1.4.5 pending explicit instruction.
>
> Last updated: 2026-08-09 — **v1.4.13 → v1.4.23 — Songs player: queue, shuffle, playlists, downloads, background audio. dev/qat only; PROD HELD (still on v1.4.5).** analyze clean · **523/523 tests**.
> • **🔴 KNOWN BROKEN: web/PWA audio is SILENT on iOS.** Reported three times from a real iPhone. The media session works perfectly — lock screen, Control Centre, artwork, advancing position, working transport — and no sound comes out. **Root cause is understood**: `audioplayers_web` routes audio through the Web Audio API (`createMediaElementSource` in `wrapped_player.recreateNode`), so nothing is audible unless its `AudioContext` is running; iOS only permits `AudioContext.resume()` inside a user gesture, and every `await` between the tap and that call spends the activation. v1.4.23 removes every await *we* control (`_playCurrent` issues `play()` synchronously, `setQueue` starts playback before publishing the OS queue) — **but audioplayers' own `play()` still awaits `setSource()` before reaching `element.play()`, and that is inside the package.** If iOS is still silent, the fix is a web-only playback engine using a plain `HTMLAudioElement` with no AudioContext in the path. **Native audio is fine.** Ruled out along the way: silent switch, volume, Bluetooth routing, audio_service (v1.4.22 disabled it on web and the silence persisted — that change is now questionable and could be reverted if web Media Session is wanted back), and the AVAudioSession category (v1.4.21 — native-only, and audioplayers already defaults to `respectSilence:false` → `playback`, so it was never the missing piece).
> • **Catalogue is 606 songs across FOUR sites** (fydt 213, CDC 283, cgdc.hk 63, Cahaya 47), published on yswords-data and fetched at runtime — new songs reach users without an app build. **559 are shown**: `SongService.hiddenSources` hides Cahaya because all 47 of its songs live on SoundCloud/YouTube with no stream URL a player can open. That is a presentation decision; the data stays complete and a test asserts it. Emptying that set restores them if Cahaya ever hosts its own mp3s.
> • **cgdc.hk years are DISCOVERED, not listed** — `wp/v2/pages` filtered by the `\d{4}mk` slug pattern, so next Easter's songbook joins on its own. ⚠️ The printed 2021 and 2022 songbooks carry QR codes to `cgdc.hk/2021mk` and `/2022mk`; **both 404** and those pages are not on the site. Worth telling the church.
> • **`scripts/sync_songs.py` has a per-source collapse guard.** A run where fydt.org timed out once produced 393 songs with all 213 fydt entries marked "no longer upstream" — the old guard only refused a *totally* empty result. Losing >50% of a source now aborts with the previous catalogue intact. It fired correctly the first time it was needed.
> • **The bundled snapshot is PULLED from yswords-data** (`scripts/pull_songs_snapshot.py`), not re-scraped. Running the full sync in both repos doubled the load on small church servers — fydt.org firewalled this machine mid-session and still times out here (the site is up; verified via an external fetcher). GitHub Actions has a different egress IP, so the weekly cron is unaffected.
> • **Web playback needs the `/song-media/*` proxy in netlify.toml — do not remove it.** `audioplayers_web` hard-codes `crossOrigin='anonymous'` and none of the four church servers send `Access-Control-Allow-Origin`. A test pins every playable host to a proxy rule. The permanent fix is asking the four sites to send that one header, after which the proxy can go.
> • Player: `SongAudioHandler` (audio_service) + `SongQueue` (shuffle reorders ONCE so "previous" is meaningful; repeat off/all/one; dead URLs drop out). Playlists with favourites, rename, and smart playlists that save a FILTER not a snapshot. Per-playlist track preference — 「伴奏 · 没有的跳过」 is what makes a quiet driving playlist trustworthy. Native downloads with queue, progress, resume-safe `.part` writes and space accounting.
>
> Last updated: 2026-08-09 — **v1.4.12 → v1.4.13 — Songs directory REBUILT. dev/qat (4 sites) only; PROD HELD.** analyze clean · **459/459 tests**.
> • **The Songs feature is back, on real APIs.** It was deleted in v1.3.126 ("the song links were reported broken") because the old sync scraped fydt.org's WordPress *sitemap* and re-parsed page markup — then fydt.org migrated its backend (its own APK changelog: 福音电台后台迁移改造) and the whole catalogue silently rotted. `scripts/sync_songs.py` is rewritten against **fydt.org's `fydt-api/v1` JSON API** (enumerate over pinyin A–Z, backstopped by a diff against the canonical `wp/v2/song` list — one legacy entry has no pinyin term; `wp/v2` also supplies the `song_category` taxonomy and ACF `bible_reference` the custom API omits). Added the **Indonesian** catalogue from cahayapengharapan.org (`/pujian/` audio + `/pujian/video-pujian/` video, merged on title). **543 songs** (fydt 213, CDC 283, Cahaya 47) across zh/en/id — 508 audio, 118 video, 510 scores, 213 lyrics. Every row shows its source.
> • **⚠️ fuyindiantai.org is NOT a separate source and must not be added as one** — but as of **2026-09-08 it is the catalogue's live hostname, and its DNS is fixed.** It is the same site as fydt.org; it 301'd to fydt.org through 2025, then its NS records were left pointing at `ns1/ns2.fydt.org` and Google and Cloudflare returned SERVFAIL (the state this line used to describe). That is over: the name resolves, `wp/v2/song` now reports `https://fuyindiantai.org/` as the site's home in every `link` and `guid`, and the 2026-09-08 weekly sync accordingly moved all 744 of that source's media URLs onto it. Both hostnames serve the identical bytes (`S03_006.mp3` → `206`, `Content-Range: bytes 0-99/4726227` on each) and neither sends `Access-Control-Allow-Origin`. **Fix applied:** `SongPlayerService._webProxyPrefixes` maps both names to the one `/song-media/fydt/` prefix and `netlify.toml` keeps its single rule pointing at `fydt.org` — same paths, and the name whose DNS has not failed here. `test/song_model_test.dart`'s host literal lists both. Do not give it a rule of its own; do not treat its songs as new.
> • **Link rot is now the pipeline's job, not the user's.** CDC's media URLs are *derived* from the catalogue code rather than published, so they are HEAD-checked during sync and dead ones dropped — **23 of 566 were never uploaded upstream**. `--verify` checks the whole catalogue (the weekly workflow runs it and fails loudly). fydt and Cahaya verify **100% clean**; all 1233 surviving URLs return 200. `test/song_model_test.dart` asserts against the REAL `assets/songs.json`, so a bad sync fails CI.
> • **🔴 v1.4.13 — web audio needs the `/song-media/*` proxy. DO NOT remove those netlify.toml rules.** `audioplayers_web` hard-codes `crossOrigin = 'anonymous'` on its `<audio>` element (`wrapped_player.dart:78`, no opt-out), so every web playback is CORS-checked — and **none of the three church servers send `Access-Control-Allow-Origin`** (verified all three). v1.4.12 shipped without this and every play button failed with `AudioPlayerException(WebAudioError, Failed to set source)`. Fix: `/song-media/{fydt,cdc,cahaya}/*` proxy rules (status 200, above the SPA catch-all like the auth proxy) make the request same-origin so CORS never applies. `SongPlayerService.resolvePlaybackUrl` rewrites **only on `kIsWeb`** — native streams direct so ~5 MB mp3s stay off our bandwidth. A test pins every playable host to a proxy rule.
> • **`audioplayers` is back** (it left in v1.3.19 with 朗读). The old model stored `audioUrl`/`pdfUrl` unused, pending authorisation "if/when the church authorises redistribution" — the pastors who publish all three sites have now approved in-app use. Nothing is rehosted; the About-page licence line was corrected from the now-false "link-out only · no audio / lyrics / PDFs are embedded".
> • Verified on dev: playback (vocal + instrumental), 206 range streaming through the proxy, detail sheet (mixes / MV / score / lyrics), search, and pruned-link rows correctly showing a language badge instead of a dead play button.
>
> Last updated: 2026-08-03 — **v1.4.0 → v1.4.5. ALL 6 sites incl. PROD on v1.4.5 (user approved).** analyze clean · **422/422 tests**.
> • **v1.4.0 — Import feature + Bible-version cleanup.** `lib/services/import_service.dart` + `MainProvider.importMergedData` + Settings UI (round-trips the existing export). Separately, **CUV / CNV / LJK1 (biblexg v1) were DELETED outright** — assets, `bible_versions.dart` entries and copyright rows — superseded by `cuvs-yhwh` / `biblexg-v2`. ⚠️ Old shared links using those version codes no longer resolve; this was the user's explicit choice over a footnote-only fix.
> • **v1.4.1–1.4.3 — "premium 丝滑" pass (part 1).** New `lib/constants/motion.dart` (`AppMotion.fast/standard/slow` + `enter/exit/symmetric` curves) replacing 21 ad-hoc `Curves.*`/`Duration` sites; new `lib/utils/app_nav.dart` `pushPage()` centralising ~58 `Get.to(...)` call sites; chapter-swipe rubber-band (`PageScrollPhysics(parent: BouncingScrollPhysics())`).
> • **🔴 v1.4.4 — THE BIG ONE: `pushPage()` silently killed navigation app-wide. READ THIS BEFORE TOUCHING `app_nav.dart`.** Symptom: after the FIRST navigation, most taps became dead — reading-pane ⋮ → Settings / Library, the 出埃及记 book picker, many buttons. **Root cause:** GetX does `routeName ??= "/${page.runtimeType}"` where `page` is the BUILDER CLOSURE, then `if (preventDuplicates && routeName == currentRoute) return null;`. The old per-call-site `Get.to(() => SettingsPage())` gave each closure a distinct type (`() => SettingsPage`) → unique names. Routing through a helper whose parameter is typed `Widget` makes EVERY closure `() => Widget` → every page resolved to `/Widget`, so after the first push `currentRoute` already matched and every later push was dropped on the floor. **Fix:** `routeName: routeName ?? '/${page.runtimeType}'` (derive from the WIDGET, never the closure). Guarded by `test/app_nav_route_name_test.dart` — verified to FAIL against the buggy line. **Debugging lesson:** this was mis-diagnosed twice (blamed `PopupMenuItem.onTap`, then `InkSparkle.splashFactory`) because agent testing entered the reader via a **deep-link URL**, which passes an explicit `routeName: '/HomePage'` and therefore MASKS the bug. Always reproduce via the real flow (Home → Read Bible → ⋮), not a deep link.
> • **v1.4.5 — "丝滑" pass (part 2), app-wide.** (1) New `lib/utils/app_scroll_behavior.dart` wired into `main.dart`'s `ScrollConfiguration`: bouncy physics + `AlwaysScrollableScrollPhysics` + no overscroll glow for **every** scrollable at once (was per-widget opt-in covering only 6 of ~27 list files). `dragDevices` deliberately left at the default — adding `mouse` would break click-drag text selection. (2) `splashFactory: InkRipple.splashFactory` on both themes so the ripple is identical on web and native (Flutter's M3 default only uses InkSparkle off-web). (3) `PressScale` **rewritten to be safe to wrap around anything** — it now drives the scale from a `Listener` (never joins the gesture arena) instead of a `GestureDetector`, so a wrapped card's own `InkWell` keeps its tap; applied to the Evidence / Sermons / Trivia card classes via a `build → _buildCard` rename (no paren surgery). Guarded by `test/press_scale_test.dart` (incl. "does not swallow the child's onTap" + "releases on scroll").
> • **Google Sign-In broke on prod the moment v1.4.x landed there — `Error 400: redirect_uri_mismatch` (fixed 2026-08-04).** The redirect-scheme rework (`authDomain: Uri.base.authority`, `cloud_auth_service.dart` `_webOptions()`) had only ever been deployed to dev/qat, whose `/__/auth/handler` redirect URIs were registered at the time; **prod's was never added**, so prod kept working only because it was still on the old `ysword.firebaseapp.com` authDomain. Deploying v1.4.x to prod switched its authDomain to `yswords.netlify.app` and Google rejected the unregistered URI. Note the asymmetry that made it easy to miss: prod WAS already in *Authorised JavaScript origins* — only the *Authorised redirect URI* was absent. Fixed by adding `https://yswords.netlify.app/__/auth/handler` in the Cloud Console; user confirmed prod sign-in works. **Rule: before shipping a NEW domain onto the redirect scheme, register that domain's redirect URI first** — `docs/google-signin-troubleshooting.md` warns about this and it still got missed.
> • **Ops lesson (this session):** a Netlify `restoreSiteDeploy` rollback is **not** a surgical revert — rolling prod back to v1.4.0 to undo the nav bug also silently reinstated the old 耶和华 wording in Bible Evidence (the 雅伟 fix had shipped in between). Check what ELSE rides along before restoring an old deploy.
>
...
> Repo: https://github.com/SuyangLiuPaul/Yahwehs-Words
