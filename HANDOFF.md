...
> Last updated: 2026-05-24 — **v1.2.95 → v1.3.18 — PageView root-cause refactor, themed iOS/Android/macOS/web app icons, per-category notification scheduler, daily-verse rotation fix, comprehensive perf sweep, navigation dedup, release tooling, Chirp 3 HD TTS + cross-platform native TTS fallback, YHWH name restoration across all aux assets, version-gap UI, Chinese exegesis prompt fix, split-pane chrome cleanup, paragraph-group Selector + scroll-state ValueNotifier refactor, haptic feedback + macOS Cmd shortcuts.**
>
