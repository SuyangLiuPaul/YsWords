...
- [x] **DONE 2026-09-13 — `assets/tagged/cuvs-yhwh/` is no longer stale
      ...
      elsewhere in the same corpus; "約"+"的話"/H1697 the same at
      Deuteronomy 29:1/29:9 and the very next verse, Jeremiah 11:3) and
      passed through an adversarial refuter before committing: 利未記
      8:14, 列王紀上 15:31, 歷代志下 18:18, 以西結書 10:1, 列王紀下 13:10,
...
- [x] **DONE 2026-09-09 — `assets/tagged/cuvs-yhwh-tr/` regenerated from
      ...
      premise ("regenerating should bring `verified` back up") was wrong
      and proven so before acting on it: `verified` increments on
      `s_tag == s_read`, and neither of those is the derived output, so a
      stale SOURCE (`assets/tagged/cuvs-yhwh/`) cannot be fixed by
      regenerating what is DOWNSTREAM of it. Regenerating the derived
      layer from the current (already-repaired) reading assets before
      touching the source produced 0 files different from the shipped
      asset — the derived layer was never behind; only
      `assets/tagged/cuvs-yhwh/` was, and that is the item above. Once
      the source was repaired (6 of 7 words), regenerating in the same
      commit brought `tagged_traditional_derived_test.dart`'s `verified`
      27,300 -> 27,306.
