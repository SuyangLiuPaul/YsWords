...
# 2026-09-09 (review finding 5): the bundled changelog is regenerated by
# `tools/bump_version.sh`, which the `if [[ "$BUMP" = "1" ]]` block
# above already ran — do NOT add a second call here.
#
# `tools/build_changelog.py` shipped on 2026-09-09 and nothing ran it,
# so assets/changelog.json was frozen at whatever the one hand run left
# behind and the app's 「更新记录」 page shipped that same stale list at
# every web release. The first fix called the generator right here,
# which covered this script and left `BUMP_VERSION=1
# yswords-ios-reinstall.sh` and a bare `bump_version.sh` still moving
# pubspec without it. It now lives with the bump instead — one place,
# every door.
#
# It also has to stay out of the `--no-bump` path: docs/release-policy.md
# step 4 exists so prod gets the SAME build dev and qat verified, and a
# regenerate here would rebuild the asset from a HEAD that has moved (at
# minimum by the dev/qat release commit) and re-date it if prod goes out
# the next day. Whatever is in the tree is what dev and qat ran.
#
# Order is still load-bearing and is still satisfied: the bump, and so
# the regenerate, happens before the first `flutter build web` — an
# asset refreshed after the bundle is written is not in the bundle.
# tools/test_build_changelog.py asserts all of this.
...
verify_site() {
  ...
  for attempt in 1 2 3; do
    ...
    # The 5 s is unchanged; it is a variable only so
    # tools/test_release_scripts.py can run the failing path — the one
    # worth testing — without three sites' worth of real waiting.
