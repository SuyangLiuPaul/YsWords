...
void main() {
  ...
  test(
    ...
    // `tools/repair_publisher_adoption_omissions.py` restored 8 words
    // into `assets/cuvs-yhwh.json`/`cuvs-yhwh-tr.json` that
    // `assets/tagged/cuvs-yhwh-tr/` — a derived asset, not regenerated by
    // this repair — does not carry. 6 of the 8 verses now fail the
    // `joined(src[entry.key]) == simplifiedById[id]` guard above and drop
    // to the weaker checks; `failures` stayed empty and `derived` stayed
    // 31,042, so nothing mismatched or went missing — these 6 simply
    // stopped being eligible for the strongest check. Queued: regenerate
    // `assets/tagged/cuvs-yhwh-tr/` from the repaired source assets
    // (docs/autonomous-queue.md, P0).
    expect(verified, 27300);
  });
